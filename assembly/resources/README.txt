...
You must be using Java 7, to check your version run:

$ java -version

And confirm that the version number starts with 1.7.

Before you can use the new Smart Config interface you must repackage the TI Smart Config library.
...
Once this is done you can run the Smart Config interface by double clicking net.betaengine.smartconfig.jar or like so from the command line:

$ java -jar net.betaengine.smartconfig.jar

Note: running executable jars by double clicking them may be disabled in certain environments in which case you will need to run them from the command line.
